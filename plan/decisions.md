...
- **D20ig. Detached camera navigation is binding** — Camera rotation, forward or backward movement, and zoom remain available after an explicit entrypoint or neighborhood-anchor retarget; selecting a node may update a secondary board reference or suggested target, but it does not tether subsequent navigation to that occurrence.
- **D20ih. Purge-first simplification is binding** — When viewer or runtime structure does not directly serve the detached-camera one-object requirements, it is simplified or removed rather than preserved alongside the target interaction model; stale explicit click-expansion or duplicate anchor-coordinate seams from the recent camera-demand iterations are removed instead of carried forward.
...
- **D20z. Traversal-first proof slice is binding** — Before anchored presets, transposition layers, or end-state budgets are treated as evidence of success, the plan must land one live detached-traversal slice in which camera travel itself reveals procedurally generated detail, geometry carries move and salience clues strongly enough to pursue a line without a sidebar-first workflow, and the board reference remains confirmatory secondary material.
...
- **O19. Detached traversal control grammar** — Exact control blend for forward or backward travel, orbit, drift, zoom, and optional line-follow assist in the live geometry-first traversal slice.
