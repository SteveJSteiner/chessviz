...
- **N11e runtime resolution/bootstrap surfaces:** viewer `apps/viewer/src/viewer/bootstrap.ts`, `apps/viewer/src/viewer/navigation.ts`, `apps/viewer/src/viewer/runtimeKernel.ts`, `apps/viewer/src/App.tsx`, and dedicated resolver/procedural-expansion modules; builder bootstrap materialization remains builder-owned.
- **N11f fixture-demotion/hard-fail surfaces:** viewer `apps/viewer/src/viewer/fixtureArtifacts.ts`, `apps/viewer/src/viewer/workspaceBoundaries.ts`, `apps/viewer/src/App.tsx`, and review/test helpers; builder fixture export remains a test-only utility alongside acceptance hard-fail wiring.
...
- **N11e**: Viewer bootstrap, navigation, runtime-kernel integration, and dedicated resolver/procedural-expansion modules resolve opening-covered positions to opening-table, supported terminal-material positions to endgame-table, and all other positions to live middlegame procedural expansion; bootstrap and entrypoint materialization no longer derive from phase labels over fixture manifests.
- **N11f**: Fixture-owned bootstrap, scene, and entrypoint paths across viewer harness modules and builder fixture-export utilities are test-only, hard-fail acceptance exists for missing required assets or regime bypass, and N12/N13/N14 consume only regime-backed runtime surfaces.
