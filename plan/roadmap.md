...
- **N00** (depends on: none): Establish module skeleton and interface boundaries.
...
- **N09** (depends on: N08): Add capture-driven departure geometry.
- **N10** (depends on: N07, N08, N09): Generate carrier/render geometry (carrier form may vary).
- **N11** (depends on: N10): Implement camera grammar v1 with continuous zoom semantics.
...
`N00 -> N01 -> N02a -> N03 -> N02b -> N04 -> N05 -> N06 -> N07 -> N08 -> N09 -> N10 -> N11 -> N12 -> N13 -> N14`
...
- **N09**: Capture edges marked with stronger departure metrics than quiet moves.
- **N10**: Carrier geometry generates without topology errors under declared budgets.
- **N11**: Continuous zoom/navigation keeps one object ontology across views.
